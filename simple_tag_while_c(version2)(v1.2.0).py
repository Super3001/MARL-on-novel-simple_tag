...
note:
1. (最先更新adversary),先更新collector，再更新controller
从逻辑上应该按照这个顺序，从实际上来说好像没有区别。
先observe再计算reward
...
2023.8.15(legion):
1. 修复了PQ不补0的bug
2. 更改了agent的顺序：adversary, collector, controller
...
DRAW_DEAD_POINT = True # 绘制已经被收集的J，为红色
...
class Scenario_new(BaseScenario):
    ...
    def make_world(self, num_adversaries=1, num_controller=1, num_collector_each=2, num_J=6, updating_J=True, envQuality='L'):
        ...
        for i, agent in enumerate(world.agents):
            ...
            agent.collector = True if (i>=num_adversaries and i<num_adversaries+num_collectors) else False
            agent.controller = True if (i>=num_adversaries+num_collectors) else False
            if agent.adversary:
                base_name = 'adversary'
                base_index = i
                agent.kind = 0
    ...
    # Agents are negatively rewarded if caught by adversaries
        rew = 0
        shape = DISTANCE_SHAPING # shape = False
        ...
        for clt in collectors:
            rew += 0.5 * clt.num_collection # 应该在更新controller之前先更新collector
        # end for
    ...
    def adversary_reward(self, agent, world):
        ...
        rew = 0
        shape = DISTANCE_SHAPING # defualt:False
    ...
    def collector_reward(self, agent, world):
        ...
        '''collecton reward'''
        rew += 1*agent.num_collection # num_collection is 0 or 1
    ...
    def observation(self, agent, world):
        if agent.kind == 0:
            ...
            agent.num_lost = 0
            for father in self.father_controller(agent, world):
                if not in_commute(agent, father):
                    agent.lost = True
                    father.num_lost += 1
                    # print('Error: son collector lost') # to inform
                else:
                    agent.lost = False
                    world.draw_line.append([agent.state.p_pos, father.state.p_pos])
            return self.collector_observation(agent, world)
        elif agent.kind == 1: # kind没有因为更新顺序而改变
            # 重置collector和controller的位置        
            return self.controller_observation(agent, world)
        else:
            raise ValueError('no such agent kind')
    ...
    def collector_observation(self, agent, world):
        ...
        priority_queue = PQ(world.N_detect)
        ...
        if len(collected_idx) > 0: # 处理收集到的资源点，并生成新的资源点
            for idx in collected_idx:
                if world.draw_dead_point:
                    world.dead_point.append(world.landmarks[idx].state.p_pos) # 目前只保存了位置
                # end if
                del world.landmarks[idx] # 删去该资源点
            print(len(world.landmarks))
            # 生成新的资源点
            if world.updating_J:
                for i in range(len(collected_idx)):
                    new_landmark = self.generate_landmark(agent, world)
                    world.landmarks.append(new_landmark)
                    priority_queue.push(new_landmark.state.p_pos, dist(new_landmark, agent))
                # end for
                print(len(world.landmarks))
